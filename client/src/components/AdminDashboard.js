...
const ClassModal = ({ teachers, existingClasses, initialData, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    const checkDuplicate = () => {
      if (!formData.className) { setDuplicateError(""); return; }
      ...
      if (isDuplicate) setDuplicateError(`Name "${formData.className}" is already taken!`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onSuccess();
    } catch (err) { alert("⚠️ Error saving class"); }
  };
           ...
           <div className="detail-card">
             ...
             {cls.meetingLink ? (
                 <div style={{display:'flex', flexDirection:'column', gap:'10px'}}>
                    <div style={{background:'#f1f5f9', padding:'8px', borderRadius:'4px', fontSize:'0.85rem', color:'#64748b', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{cls.meetingLink}</div>
                    <button onClick={handleCopyLink} className="save-btn" style={{display:'block', width:'100%', textAlign:'center', backgroundColor: copyStatus.includes('✅') ? '#10b981' : '#0284c7', transition: 'background-color 0.2s'}}>{copyStatus}</button>
                 </div>
          ...
          <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '8px', marginBottom: '20px', marginTop: '15px' }}>
            {user.role === 'parent' ? (
                <div className="edit-form-grid">
                   ...
                   <div className="form-group"><label style={{color:'#ea580c'}}>Fee Start Date (Reg)</label><input type="date" name="registeredDate" value={formData.registeredDate || ''} onChange={handleChange} style={{border:'1px solid #ea580c'}} /></div>
                </div>
            ) : user.role === 'teacher' ? (
                <div className="edit-form-grid">
...
const AddUserTab = () => {
  const [formData, setFormData] = useState({ 
    ...
    childClass: "", monthlyFee: "", specialization: "", education: "", 
          ...
          <div className="form-section student-section">
            ...
            <div className="form-row">
                ...
                <div className="form-group">
                    ...
                    <input type="date" name="registeredDate" value={formData.registeredDate} onChange={handleChange} style={{border:'2px solid #ea580c'}} />
                </div>
            </div>
...
const GalleryRepositoryTab = () => {
  ...
  const filteredArtwork = artwork.filter((art) => {
    ...
    const monthMatch = filterMonth === "all" || artMonth === parseInt(filterMonth);
  ...
  const handleDelete = async (e, artId) => {
    ...
    if (window.confirm("Are you sure you want to delete this artwork?")) {
      try {
        await axios.delete(`https://art-portal-7n6r.onrender.com/api/gallery/${artId}`);
  ...
  // Lightbox Navigation
  const handleNext = (e) => { if(e) e.stopPropagation(); setLightboxIndex((prev) => (prev + 1) % filteredArtwork.length); };
  const handlePrev = (e) => { if(e) e.stopPropagation(); setLightboxIndex((prev) => (prev - 1 + filteredArtwork.length) % filteredArtwork.length); };
  ...
  return (
    <div className="form-wrapper">
      ...
      <div style={{background: '#f8fafc', padding: '15px', borderRadius: '12px', border: '1px solid #e2e8f0', marginBottom: '20px'}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'15px'}}>
            ...
            <div style={{display:'flex', gap:'10px', flexWrap:'wrap'}}>
                {/* Student Selector */}
                <select value={selectedStudent} onChange={e => setSelectedStudent(e.target.value)} style={{padding:'8px', borderRadius:'6px', border:'1px solid #cbd5e1', fontWeight:'bold', minWidth:'180px'}}>
                ...
                {/* Year Filter */}
                <select value={filterYear} onChange={e => setFilterYear(e.target.value)} style={{padding:'8px', borderRadius:'6px', border:'1px solid #cbd5e1', cursor:'pointer'}}>
                    <option value="all">All Years</option>
                    {years.map(y => <option key={y} value={y}>{y}</option>)}
                </select>

                {/* Month Filter */}
                <select value={filterMonth} onChange={e => setFilterMonth(e.target.value)} style={{padding:'8px', borderRadius:'6px', border:'1px solid #cbd5e1', cursor:'pointer'}}>
      ...
      <div style={{marginBottom:'15px', fontSize:'0.9rem', color:'#64748b'}}>
         ...
         <span style={{color:'#2563eb', fontWeight:'bold'}}> {filterMonth === "all" ? "All Months" : months[filterMonth]} {filterYear === "all" ? "" : filterYear}</span>
      </div>

      {/* GALLERY GRID */}
      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(200px, 1fr))', gap:'20px'}}>
        {loading ? <p>Loading Gallery...</p> : 
         filteredArtwork.length === 0 ? <div style={{gridColumn:'1/-1', textAlign:'center', padding:'40px', background:'#f1f5f9', borderRadius:'12px', color:'#94a3b8'}}>No artwork found for this filter.</div> : 
         filteredArtwork.map((art, index) => (
            <div key={art._id} style={{border:'1px solid #e2e8f0', borderRadius:'12px', overflow:'hidden', background:'#fff', position:'relative', cursor:'pointer', boxShadow:'0 2px 5px rgba(0,0,0,0.05)'}} onClick={() => setLightboxIndex(index)}>
               <img 
                 src={art.imageUrl} 
                 alt={art.title} 
               ...
               <div style={{padding:'12px'}}>
                  {selectedStudent === 'all' && (
                     <div style={{fontSize:'0.75rem', fontWeight:'bold', color:'#2563eb', marginBottom:'4px', textTransform:'uppercase'}}>
                       {art.studentId?.childName || "Unknown"}
                     </div>
                  )}
                  <div style={{fontWeight:'bold', color:'#334155'}}>{art.title || "Untitled"}</div>
                  <div style={{fontSize:'0.8rem', color:'#64748b'}}>{art.medium} • {new Date(art.dateCreated).toLocaleDateString()}</div>
               </div>
               <button 
                 onClick={(e) => handleDelete(e, art._id)}
                 style={{position:'absolute', top:'10px', right:'10px', background:'rgba(255,255,255,0.9)', border:'none', borderRadius:'50%', width:'30px', height:'30px', cursor:'pointer', color:'#ef4444', fontWeight:'bold', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 2px 5px rgba(0,0,0,0.2)'}}
      ...
      {lightboxIndex !== null && filteredArtwork[lightboxIndex] && (
        <div className="lightbox-overlay" onClick={() => setLightboxIndex(null)}>
          ...
          <div className="lightbox-content" onClick={(e) => e.stopPropagation()}>
            <img src={filteredArtwork[lightboxIndex].imageUrl} alt={filteredArtwork[lightboxIndex].title} className="lightbox-img" />
          </div>
          <button className="lightbox-nav-btn next" onClick={handleNext}>›</button>
          <div className="lightbox-caption">
            {selectedStudent === "all" && <h2 style={{margin:'0 0 5px 0', fontSize:'1.4rem'}}>{filteredArtwork[lightboxIndex].studentId?.childName}</h2>}
    ...
    <div className="admin-container">
      <aside className="admin-sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          <button className={activeTab === "gallery" ? "active" : ""} onClick={() => setActiveTab("gallery")}><span style={{fontSize:'1.2rem'}}>🎨</span> <span>Gallery</span></button>
        </nav>
      ...
      <main className="admin-main">
        <header className="top-header">
          <div className="header-title">
            <h2>{activeTab === "overview" ? "Overview" : activeTab === "users" ? "All Users" : activeTab === "classes" ? "Class Management" : activeTab === "fees" ? "Fee Tracker" : activeTab === "slots" ? "Slot Manager" : activeTab === "gallery" ? "Art Gallery" : "Register User"}</h2>
      ...
      <nav className="mobile-bottom-nav">
        ...
        <button className={activeTab === 'slots' ? 'nav-item active' : 'nav-item'} onClick={() => handleNavClick('slots')}>
          <IconClock /> <span>Slots</span>
        </button>
        {/* ✨ ADDED GALLERY BUTTON MOBILE */}
